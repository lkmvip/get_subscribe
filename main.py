...
def get_subscribe_url():
    ...
    if not summary:
        ...
        return
    v2ray_list = re.findall(r"v2ray\(请开启代理后再拉取\)：(.+?)</div>", summary)
    # 获取普通订阅链接
    if v2ray_list:
    ...
    clash_list = re.findall(r"clash\(请开启代理后再拉取\)：(.+?)</div>", summary)
    # 获取clash订阅链接
    if clash_list:
        ...
        if clash_code not in ok_code:
            ...
            with open(dirs + '/clash.yml', 'w', encoding="utf-8") as f:
                ...
                clash_content = clash_content.replace('https://www.mattkaydiary.com|', "")
                clash_content = clash_content.replace('mattkaydiary.com|', "")
                clash_content = clash_content.replace('/小东的程序未能精准识别(可能是中转节点)', "")
